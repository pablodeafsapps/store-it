...
description: >-
  ...
  repo (composeApp common/android/ios vs androidApp). Use when adding or moving
...
| Module | Role |
|--------|------|
| `:composeApp` | Shared KMP library: domain, data, shared Compose UI, DI init, SQLDelight. Ships a static `ComposeApp` framework to iOS. |
| `:androidApp` | Android application entry: navigation shell, Android-only Compose screens, `implementation(projects.composeApp)`. |
...
- Koin modules and metadata; KSP output under `build/generated/ksp/metadata/commonMain/kotlin` (already wired in `composeApp/build.gradle.kts`).
...
- Dependencies that are JVM/Android-only (see `androidMain.dependencies` in `composeApp/build.gradle.kts`).
...
Prefer **`composeApp/src/commonTest`** for logic that is truly shared and has no Android dependency.
...
3. **Is it UI shown on both platforms from shared code?** → `composeApp` `commonMain` (and platform resources only if truly platform-specific).
...
- SQLDelight config: `composeApp/build.gradle.kts` (`sqldelight { databases { … } }`).

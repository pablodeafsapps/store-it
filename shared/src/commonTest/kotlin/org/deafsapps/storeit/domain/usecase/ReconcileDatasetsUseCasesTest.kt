...
internal class ReconcileDatasetsUseCasesTest {
  ...
  @Test
  fun `GIVEN blank account id WHEN keep-local selected THEN validation error is returned`() =
      runTest {
  ...
  @Test
  fun `GIVEN blank account id WHEN keep-remote selected THEN validation error is returned`() =
      runTest {
  ...
  fun `GIVEN account id WHEN keep-local selected THEN decision is saved and applied`() = runTest {
    ...
    assertEquals(
        expected = ReconciliationDecisionType.KeepLocal,
        actual = fakeRepository.savedDecision?.decisionType,
    )
    assertEquals(
        expected = ReconciliationDecisionType.KeepLocal,
        actual = fakeRepository.appliedDecisionType,
    )
  }
  ...
  fun `GIVEN account id WHEN keep-remote selected THEN decision is saved and applied`() = runTest {
    ...
    assertEquals(expected = DataMode.AccountBackedPendingSync, actual = result.getOrNull())
    assertEquals(
        expected = ReconciliationDecisionType.KeepRemote,
        actual = fakeRepository.savedDecision?.decisionType,
    )
    assertEquals(
        expected = ReconciliationDecisionType.KeepRemote,
  ...
  @Test
  fun `GIVEN decision save fails WHEN keep-local selected THEN error is returned and apply decision is skipped`() =
      runTest {
  ...
  @Test
  fun `GIVEN apply decision fails WHEN keep-remote selected THEN error is returned after decision is saved`() =
      runTest {
...
private class FakeReconciliationRepository : ReconciliationRepository {
  ...
  override suspend fun getReconciliationSummary(
      accountId: String
  ): Result<DomainError, ReconciliationSummary> =
  ...
  ): Result<DomainError, DataMode> = applyDecisionResult.also { appliedDecisionType = decisionType }
}

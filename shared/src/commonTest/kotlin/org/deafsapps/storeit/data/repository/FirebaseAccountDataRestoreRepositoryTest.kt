...
class FirebaseAccountDataRestoreRepositoryTest {
    ...
    @Test
    fun `GIVEN first account restore snapshot with photos WHEN restoreAccountData THEN applies remote dataset locally and records synchronized metadata`() =
        runTest {
    ...
    @Test
    fun `GIVEN stale local account-backed data WHEN restore succeeds THEN clears local repositories before applying authoritative remote snapshot`() =
        runTest {
            val session = accountSession()
            val snapshot = remoteAccountSnapshot()
            fakeAccountRemoteDataSource.fetchSnapshotResult = snapshot.ok()
